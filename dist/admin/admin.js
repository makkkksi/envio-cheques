...
function pedirConfirmacionRecepcion(id, numFactura) {
    ...
    document.getElementById('formCompletarCheques').onsubmit = (e) => {
        ...
        for (let i = 0; i < bancosInputs.length; i++) {
            ...
            const emitido_a = emitidoInputs[i] ? emitidoInputs[i].value.trim() : 'Automarco LTDA';
            
            if (!banco || !num || !ctaCorriente || isNaN(monto) || monto <= 0) {
                showToast('Debe completar Banco, N° de Cheque, Cuenta Corriente y Monto válido para todos los documentos.', 'error');
                valid = false;
                break;
            }
