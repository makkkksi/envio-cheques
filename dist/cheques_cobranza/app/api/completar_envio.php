...
try {
    ...
    // --- [SEC-01] Blindaje IDOR: En producción, validar que la cobranza pertenezca al vendedor en sesión ---
    if (defined('APP_ENV') && APP_ENV === 'production') {
        ...
        $vendedor_sesion_id = (int) $_SESSION['vendedor_auth']['vendedor_id'];
        $sql .= ' AND (c.vendedor_id = :uid OR c.vendedor_id IS NULL)';
        $params[':uid'] = $vendedor_sesion_id;
    } elseif (!(defined('APP_ENV') && APP_ENV === 'local' && $usuario_id === 1)) {
        $sql .= ' AND (c.vendedor_id = :uid OR c.vendedor_id IS NULL)';
