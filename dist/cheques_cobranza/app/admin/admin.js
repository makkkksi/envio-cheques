...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' || e.keyCode === 27) {
        if (typeof cerrarModalConfirmacionRecepcion === 'function') cerrarModalConfirmacionRecepcion();
        if (typeof cerrarModalDeposito === 'function') cerrarModalDeposito();
        if (typeof cerrarModalRechazo === 'function') cerrarModalRechazo();
    }
});
...
function renderSidePanelDetail(data) {
    ...
    if (tieneMismatch) {
        if (!boxDiscrepancia) {
            ...
            document.querySelector('.detail-info-grid').parentNode.insertBefore(boxDiscrepancia, document.querySelector('.detail-info-grid'));
        }

        const deltaFmt = '$' + Math.abs(deltaVal).toLocaleString('es-CL');
        const tipoDelta = deltaVal < 0 ? 'Diferencia a favor del Cliente' : 'Excedente en Cheques';
    ...
    if (cheques.length === 0) {
        ...
        boxCheques.innerHTML = btnEditar + cheques.map(chq => {
            const emitidoTexto = chq.emitido_a || cob.empresa_nombre || 'Automarco LTDA';
            ...
            <div class="cheque-inspection-card" id="chqView_${chq.id}">
                ...
                <div class="cheque-card-meta">
                    ...
                    ${chq.comentario ? `
                        <div class="quote-comment" style="margin-top: 4px;">
                            "${chq.comentario}"
                        </div>
                    ` : ''}
...
function pedirConfirmacionRecepcion(id, numFactura) {
    ...
    const bancosList = ['Banco de Chile', 'Banco Santander', 'Banco BCI', 'Banco Estado', 'Scotiabank', 'Itaú', 'Banco Security', 'Banco BICE', 'Banco Internacional', 'Banco Consorcio', 'Banco Falabella', 'Banco Ripley', 'BCIPREMIER', 'Otro'];
    const defaultEmpresa = (window.cobranzaActiva && window.cobranzaActiva.empresa_nombre) ? window.cobranzaActiva.empresa_nombre : 'Automarco LTDA';
    ...
    window.chequesActivos.forEach((chq, index) => {
        ...
        const emitidoActual = chq.emitido_a || defaultEmpresa;

        const fotoModalHtml = chq.foto_cheque_url ? `
                ...
                <div style="flex: 1; display: grid; gap: 10px;">
                    ...
                    <div>
                        ...
                        <select class="input-emitido-tesoreria" data-id="${chq.id}" style="width: 100%; padding: 6px; border: 1px solid #cbd5e1; border-radius: 4px;" required>
                            ...
                            <option value="Automarco LTDA" ${emitidoActual === 'Automarco LTDA' ? 'selected' : ''}>Automarco LTDA</option>
                            <option value="HD Automarco S.A." ${emitidoActual === 'HD Automarco S.A.' ? 'selected' : ''}>HD Automarco S.A.</option>
                            <option value="Autotec S.A." ${emitidoActual === 'Autotec S.A.' ? 'selected' : ''}>Autotec S.A.</option>
                            <option value="Gabtec S.A." ${emitidoActual === 'Gabtec S.A.' ? 'selected' : ''}>Gabtec S.A.</option>
                            <option value="Otro" ${emitidoActual === 'Otro' ? 'selected' : ''}>Otro</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        <label style="display: block; font-size: 0.8rem; margin-bottom: 4px;">Cuenta Corriente *</label>
                        <input type="text" class="input-cta-corriente-tesoreria" data-id="${chq.id}" value="${chq.cuenta_corriente || ''}" placeholder="Ej: 1234567890" style="width: 100%; padding: 6px; border: 1px solid #cbd5e1; border-radius: 4px;" required>
                    </div>
                </div>
            </div>
        `;
    ...
    modal.innerHTML = `
        <div class="modal-box" style="max-width: 600px; width: 90%; max-height: 90vh; display: flex; flex-direction: column;">
            ...
            <p style="font-size: 0.85rem; color: var(--color-text-secondary); margin-top: 8px;">Por favor, digite el Banco, N° de Cheque y Cuenta Corriente para cada documento de la <strong style="color: var(--color-primary);">Factura N° ${numFactura}</strong>.</p>
    ...
    document.getElementById('formCompletarCheques').onsubmit = (e) => {
        ...
        const ctaCorrienteInputs = document.querySelectorAll('.input-cta-corriente-tesoreria');
        ...
        for (let i = 0; i < bancosInputs.length; i++) {
            ...
            const ctaCorriente = ctaCorrienteInputs[i].value.trim();
            const monto = parseFloat(montosInputs[i].value);
            const emitido_a = emitidoInputs[i] ? emitidoInputs[i].value.trim() : '';
            
            if (!banco || !num || !ctaCorriente || !emitido_a || isNaN(monto) || monto <= 0) {
                showToast('Debe completar Banco, N° de Cheque, Cuenta Corriente, Empresa Destino (Emitido a) y Monto válido para todos los documentos.', 'error');
                valid = false;
                break;
            }
            
            chequesData.push({ id: chqId, banco: banco, numero_cheque: num, cuenta_corriente: ctaCorriente, monto: monto, emitido_a: emitido_a });
        }
...
function ejecutarCambioEstado(id, nuevoEstado, extraData = {}) {
    ...
    .then(data => {
        ...
        if (nuevoEstado === 'RECIBIDO_TESORERIA') {
            showToast('Cobranza validada y enviada a Cuentas Corrientes correctamente.', 'success');
        } else {
        ...
        // NOTA: Para RECIBIDO_TESORERIA se mantiene en la bandeja actual sin saltar de tab
        ...
        // Al validar hacia C.Corrientes, deseleccionar para limpiar el inspector lateral
        if (nuevoEstado === 'RECIBIDO_TESORERIA') {
            deseleccionarCobranza();
        } else if (cobranzaSeleccionadaId == id) {
...
function activarModoEdicion(cobranzaId) {
    ...
    const bancos = ['Banco de Chile', 'Banco Santander', 'Banco BCI', 'Banco Estado', 'Scotiabank', 'Itaú', 'Banco Security', 'Banco BICE', 'Banco Internacional', 'Banco Consorcio', 'Banco Falabella', 'Banco Ripley', 'BCIPREMIER', 'Otro'];
...
// Security Hardening - Extract Inline Event Listeners
document.addEventListener('DOMContentLoaded', () => {
    const btnCerrarPanelDetalle = document.getElementById('btnCerrarPanelDetalle');
    if (btnCerrarPanelDetalle) {
        btnCerrarPanelDetalle.addEventListener('click', () => {
            if (typeof deseleccionarCobranza === 'function') {
                deseleccionarCobranza();
            }
        });
    }
