...
if ($method === 'POST' && $action === 'login') {
    ...
    $stmt = $pdo->prepare("SELECT * FROM web_usuarios WHERE usuario = ? AND activo = 1");
    ...
    $pdo->prepare("INSERT INTO web_sesiones (usuario_id, token, ip, user_agent, expira_en) VALUES (?,?,?,?,?)")
        ->execute([$user['id'], $token, $ip, $ua, $expira]);

    $pdo->prepare("UPDATE web_usuarios SET ultimo_login = NOW() WHERE id = ?")
        ->execute([$user['id']]);

    setcookie('at_token', $token, time() + SESSION_TTL, '/', '', true, true);
...
if ($method === 'POST' && $action === 'logout') {
    ...
    if ($token) {
        $pdo = getDB();
        $pdo->prepare("DELETE FROM web_sesiones WHERE token = ?")->execute([$token]);
    }
    setcookie('at_token', '', time() - 3600, '/', '', true, true);
...
if ($method === 'GET' && $action === 'check') {
    ...
    $pdo  = getDB();
    $stmt = $pdo->prepare("
        SELECT u.nombre, u.rol, u.usuario, u.cli_rut, u.cli_sec, u.vend_cod
        FROM web_sesiones s 
    ...
    $user['empresa_codigo'] = EMPRESA_CODIGO;
