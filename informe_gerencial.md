...
El script automático de despacho (`cron/resumen_diario_cuentas_corrientes.php`) implementa dos capas cruciales de control técnico:
* **Idempotencia por Base de Datos (Prevención de Duplicados):** Si el cron job del servidor se ejecuta cada minuto, el sistema valida en `log_envios_informes` que no se haya enviado un correo exitoso (`ENVIADO`) para esa empresa el día de hoy. De existir, aborta el proceso inmediatamente, garantizando que las digitadoras no reciban correos duplicados.
